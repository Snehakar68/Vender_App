...
const token = auth?.user?.token;
  ...
  const fetchDoctor = async () => {
    try {
      ...
      setForm({
        ...
        licensePreview: docs.license
          ? docs.license.startsWith("JVBER")
            ? `data:application/pdf;base64,${docs.license}`
            : `data:image/jpeg;base64,${docs.license}`
          : null,
      });
  ...
  const pickFile = async () => {
    ...
    if (!result.canceled) {
      ...
      if (picked.size && picked.size > MAX_FILE_SIZE) {
        setErrors((prev: any) => ({
          ...prev,
          license: "File size must be less than 200KB",
        }));
      ...
      const preview = picked.mimeType?.includes("pdf")
        ? base64
  ...
  return;
      ...
      if (file) {
        ...
        const fileUri =
          FileSystem.cacheDirectory + "existing_license.jpg";

        await FileSystem.writeAsStringAsync(
          fileUri,
          form.licensePreview.replace(/^data:.*;base64,/, ""),
          { encoding: "base64" }
        );
    ...
    try {
      ...
      if (isPdf) {
        ...
        if (!file.startsWith("data:")) {
          imageUri = `data:image/jpeg;base64,${file}`;
        }
