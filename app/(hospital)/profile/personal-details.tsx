...
//   // };
// // ─────────────────────────────────────────────────────────────────────────
// // UPDATED: compressToLimit
// //
// // CHANGE: Added an immediate pre-check BEFORE compression passes.
// // If the raw file is already ≤ 200KB, return it instantly (no compression).
// // If the file is over 200KB, attempt compression passes as before.
// // The null return path (still too large after all passes) is unchanged —
// // the Alert in pickImageFromCamera / pickImageFromGallery handles that case.
// // ─────────────────────────────────────────────────────────────────────────
// const compressToLimit = async (uri: string): Promise<string | null> => {
//   try {
//     // ✅ NEW: Pre-check — if already within limit, skip compression entirely
//     const rawInfo = await FileSystem.getInfoAsync(uri);
//     const rawSize = (rawInfo as any).size ?? 0;

//     if (rawSize <= MAX_IMAGE_BYTES) {
//       // Already under 200KB — use as-is
//       return uri;
//     }

//     // ✅ NEW: Show immediate warning with actual file size before attempting compression
//     const rawKB = Math.round(rawSize / 1024);
//     Alert.alert(
//       "Image Too Large",
//       `This image is ${rawKB} KB. The limit is 200 KB. Attempting to compress it automatically…`,
//       [{ text: "OK" }],
//     );

//     // Compression passes (unchanged from original)
//     let current = uri;
//     const widths = [1200, 900, 700, 500];
//     const qualities = [0.8, 0.65, 0.5, 0.35];
