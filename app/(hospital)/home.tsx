...
export default function HospitalHome() {
  ...
  // ── Load vendorId + hospital name on mount ─────────────────────────────────
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (cached) {
          const { hospitalName: name } = JSON.parse(cached);
          if (name) {
        ...
        if (userData.vendorId) {
          const res = await api.get(
            `/api/Hospital/GetHosPersonnelInfoById/${userData.vendorId}`,
          );
          const apiData = res.data?.data;
          const name: string = apiData?.hospital?.full_name ?? "";
          setHospitalName(name);
          const rawPhotos: string[] = Array.isArray(apiData?.photos)
            ? apiData.photos
            : [];
          const img =
            rawPhotos.length > 0
  ...
  const loadPendingDoctors = useCallback(async (vid: string) => {
    try {
      const res = await api.get(
        `https://coreapi-service-111763741518.asia-south1.run.app/api/Hospital/Total_pending_Doctor_Hosp?Hosp_ID=${vid}`,
      );
  ...
  const loadPendingNurses = useCallback(async (vid: string) => {
    try {
      const res = await api.get(
        `https://coreapi-service-111763741518.asia-south1.run.app/api/Hospital/Total_pending_Nurse_Hosp?Hosp_ID=${vid}`,
      );
          ...
          {STAT_CARDS.map((card) => (
            ...
            >
              ...
              <Text style={styles.statCount}>
                {counts[card.key as keyof Counts]}
              </Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.chartArea}>
            {BAR_CONFIG.map((b, i) => {
              ...
              const barH = Math.max(
                4,
                Math.round((val / maxVal) * BAR_MAX_HEIGHT),
              );
              return (
                <View key={b.key} style={styles.barColumn}>
                  ...
                  <View style={styles.barTrack}>
                    <View
                      style={[
                        styles.bar,
                        { height: barH, backgroundColor: b.color },
                      ]}
                    />
                  </View>
