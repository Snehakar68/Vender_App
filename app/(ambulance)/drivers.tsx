...
import { Driver } from '@/src/features/ambulance/data/mockData';
...
export default function DriversScreen() {
  const { drivers, deleteDriver } = useAmbulanceContext();
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((driver) => (
            <DriverCard
              key={driver.id}
              data={driver}
              onView={() => router.push(`/(ambulance)/add-driver?mode=view&id=${driver.id}`)}
              onEdit={() => router.push(`/(ambulance)/add-driver?mode=edit&id=${driver.id}`)}
